# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
from ..core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .harbor_schema_response import HarborSchemaResponse
from .pagination_details import PaginationDetails


class HarborSchemaListEnvelope(UniversalBaseModel):
    data: typing.Optional[typing.List[HarborSchemaResponse]] = None
    pagination: typing.Optional[PaginationDetails] = None
    supports_namespaces: typing.Optional[bool] = None

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
